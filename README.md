...
- **Am I about to lock up?** -- Sparkline row holds 3 of CPU / MEM / SWAP / TOK / PRTY with severity coloring (green / yellow / red). Press `1`-`5` to swap which graphs are showing; CPU + MEM + TOK are visible by default. TOK reads actual Claude Code input+output tokens/sec, PRTY is the chars÷4 estimate that mimics CC's own streaming counter.
...
Keyboard shortcuts: `h` help overlay, `q` quit, `i` session intel, `1`-`5` toggle sparklines. Press `h` for the full list.
