...
- **`claude-statusline/`** — the **per-session** surface. Current model, context window, cumulative session token totals, session cost, git branch. Rendered in every session chrome.

System-wide signals belong only in thermo — do not propose adding them to statusline even when a cell would technically fit. Statusline crowding duplicates information thermo already shows and makes the per-session surface noisy. A rolling multi-day token total is a system-wide signal by this rule, not a statusline cell. The same non-duplication rule applies to Claude Code's own chrome: effort level is not shown, because the terminal already surfaces it outside the status line.

Within the line, right-now values (model, context, branch) are kept apart from session-cumulative ones (tokens, cost). The model in particular must not sit beside the cost figure — cost is priced per message from each message's own model, and any session using subagents mixes models, so adjacency would imply a scope the number does not have.
...
Cost is therefore computed locally from that same transcript, at public list prices, per message using that message's own model. It must be priced from the separate billing buckets, never from the displayed input total — cache reads bill at 0.1x and dominate a long session, so pricing the combined input figure at the fresh-input rate overstates by ~8x. The figure is framed by provider because it means different things: off subscription it estimates real marginal spend, while on Pro/Max the fee is flat and the number is value drawn against it, not a budget.
...
Every line carries `"schema":1` (envelope shape contract — bumps on rename or removal, additive optional fields stay at 1). Writes are serialized via the non-reentrant `coolant_lock` mkdir-mutex so parallel hooks emitting >PIPE_BUF payloads can't splice. Lock-acquisition timeout falls through to an unsynchronized write and bumps `$COOLANT_DEGRADED_COUNT` (out-of-band counter, truncated on cold-start SessionStart — resume/clear skip it so a continued session's bus isn't reset). The Go schema gate at `internal/stats` filters pre-versioning events at parse time — old and new envelopes coexist in the same JSONL ("virtual chop").

A second JSONL bus carries Claude Code's beta OTEL emissions: thermo embeds an OTLP/HTTP receiver on `127.0.0.1:4318`, writes parsed metric data points to `$TMPDIR/coolant-$USER.cc-otel.jsonl`, and an in-process tailer feeds the cc-otel reconcile loop in `internal/otel/cc/`. The raw sink rotates to a single `.1` sibling at 10 MB (checked per request inside the write mutex, mirroring the findings writer) — without it the file is unbounded, and `$TMPDIR` is not a safety net: macOS only sweeps entries untouched for ~3 days and this one is written every 10 seconds. Rotation is signalled in-process (`Receiver.OnRotate` → `MetricsTailer.NoteRotation`, wired in `startCcOtel`) and fires **before** the rename, because the tailer polls on its own clock: an inode change it can't attribute is an external recreation and resets the aggregate, whereas a rotation it was told about carries the day window forward and restarts only the read offset. Without that signal the window zeroes mid-day while coolant's own `TokensInTotal` keeps climbing, and `compareTokens` files a `value_mismatch` every reconcile tick until UTC midnight. Note the receiver never reads OTLP `AggregationTemporality`, so a `cumulative` emitter re-sends the whole accumulated metric set every interval and every copy is appended; `delta` reduces the volume but bounds nothing. The reconciliation surface is read-only against CC OTEL — drift surfaces as filing-grade findings written to the durable `~/.coolant/cc-otel-findings.jsonl`. Review via `thermo cc-findings`; enable via `scripts/enable-cc-otel.sh`. The findings file persists `session.id`, `user.account_uuid`, and `organization.id` for triage; `user.email` is stripped at the receiver before reaching disk.
...
- **Marketplace:** `todd-w-shaffer/marketplace` repo hosts the plugin manifest. Coolant is referenced via a `git-subdir` source pointing at this repo's main HEAD — no automation needed; pushing to main publishes new versions immediately.
...
- **Install script:** `install.sh` downloads the binary for the user's arch, asks where to put it, and optionally installs the braille statusline to `~/.claude/` with settings.json patching. `install.sh --upgrade` delegates to `scripts/upgrade.sh` for silent re-fetch of both artifacts. The binary refresh inside `scripts/upgrade.sh` prefers `thermo upgrade` (atomic sibling-temp + `os.Rename` via `internal/upgrader/`) and falls back to in-place `curl -o` only when the existing binary can't run the subcommand — so both user-facing entry points (`thermo upgrade` directly, or `install.sh --upgrade`) converge on the atomic Go upgrader for the binary. Statusline refresh stays in bash since it's a bash artifact.
- **Staleness detection:** Daily TTL check against `VERSION` on main, cached at `$TMPDIR/coolant-$USER.latest-version`. Thermo's notification banner is the sole staleness surface and shows "update available". (The per-session statusline previously appended a dim yellow ⬆ glyph; that was removed because system-wide signals don't belong on a per-session surface.) `[updates]` TOML section controls TTL and opt-out.
